...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo  '<form action="edit2.php?name=' . $name . '" method="post">';
    echo  '<label for="type">Type:</label>';
    echo  '<input type="text" id="type" name="type" value="'.$row['Type'].'" required>';

    echo  '<label for="name">Name:</label>';
    echo  '<input type="text" id="name" name="name" value="'.$row['Name'].'" size="55" maxlength="55" required>';

    echo  '<label for="price">Price:</label>';
    echo  '<input type="text" id="price" name="price" value="'.$row['Price'].'" pattern="[0-9]+"
    title="Price Must be an Integer" required>';

    echo  '<label for="description">Description:</label>';
    echo  '<input type="text" id="description" name="description" value="'.$row['Description'].'" required>';
